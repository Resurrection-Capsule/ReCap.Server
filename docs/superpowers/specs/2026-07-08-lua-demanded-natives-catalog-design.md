...
**Wave 3 landed 2026-07-08 (7 natives) → demanded 9 → 2.** Implemented: nModifier.{GetMyAgentID,GetMyInitiatorID,GetMyStackCount,GetInitiatorAttributeSnapshot,PreloadAsset}, nAttribute.AddAttributeModifier, nGameObject.AddEffect (emits client-verified 0x9B ServerEvent, attached recipe {6,7}+9). AbilityInvocation gained InitiatorId/StackCount (== retail ModifierInvocation); Game.BroadcastServerEvent added. Plan: `docs/superpowers/plans/2026-07-08-lua-natives-wave3-modifier-fx.md`. Deferred (flagged): modifier APPLICATION subsystem (RequestModifier / when modifiers get invoked with a live initiator — nModifier getters are correct but dormant until then); Remove side (RemoveEffect/RemoveAttributeModifier); additive-vs-mult attribute modifier (additive chosen). **Harvest caveat:** the modifier enumeration now truncates (a modifier legitimately parks on a wait once natives work → harvest aborts at first stuck coroutine; started 99→13). The demanded=2 comes from the complete ability pass; a full modifier re-census needs the harvest to drain/wake parked modifier threads. Remaining 2 = Wave 4 (spawn: nObjectManager.CreateObject + AttachTriggerVolume).

**Wave 4 landed 2026-07-08 (2 natives, inline) — the original 27 are ALL implemented.** nObjectManager.CreateObject (allocate id + ObjectManager.Spawn + gated 0x8C ObjectCreate announce, enemy-shape {6,7}, gated on `NounResolves`; Ghidra-verified handler `OnGmsObjectCreate@0x0053f550` = reflection envelope, optional fields) + nObjectManager.AttachTriggerVolume (sphere + captured Lua callback refs; firing deferred — no collision system). Plan: `docs/superpowers/plans/2026-07-08-lua-natives-wave4-spawn.md`. **Frontier deepened:** implementing CreateObject let projectile scripts run further and reveal **4 NEW demanded natives** (harvest 2→4 by composition; the 2 spawn natives dropped): `nGameObject.GetAttributeSnapshot`, `nGameObject.IsCombatant`, `nObjectManager.DetachTriggerVolume`, `nThread.WaitForProjectile` — a candidate Wave 5 (all small/mechanical except WaitForProjectile). The 27-native goal is met; the demand-driven surface simply extends as scripts execute deeper.
