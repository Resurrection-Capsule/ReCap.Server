...
- **RESOLVED 2026-07-08 (Ghidra client parse — supersedes the crash-suspicion above):** the retail client's `ClientNet::OnGmsObjectivesInitForLevel` @0x0053c820 reads **`u8 count`, then per objective 56 bytes = `u32 id (4) + 4× u8 flags (4) + 48-byte blob (0x30)`** via `ClientNet::ReadStreamBytes` @0x00a8d310. So the client's real per-objective stride is **56 bytes, not 7** — C#'s 56B/objective is the *parser-faithful* form. The 37B capture reconciles because **`ReadStreamBytes` is short-read tolerant** (clamps to remaining bytes, advances by the clamped amount, returns actual count, never over-reads or asserts): the drifted prebuilt-binary's 37B packet is tolerated (objective 1 partial, 2..n zeroed) with **no crash**. **⇒ 0xB7 is NOT a crash vector at any entry width**; the earlier "high crash suspicion" is withdrawn. Only the 48B blob's field roles remain TBD. General corollary: a server sending fewer bytes than a parser reads never crashes this client (trailing fields zero-fill) — misreads that *shift* fields are the real danger, not under-length.
- **DirectorState (0x8B) field map — Ghidra-resolved 2026-07-08.** `ClientNet::OnGmsDirectorState` @0x0053dcd0 parses via the generic reflection parser (not a raw 0x4D0 blob). Class = **cAIDirector**, hash **0x1fb04a19**, struct size 0x4d0; registrar `AssetData::cAIDirector` @0x00f78480 + `AssetType::cAIDirector` @0x00f789e0 (**count=7**). The 7 reflection fields (id: name type @offset): 0 `mbBossSpawned` bool@0x0d, 1 `mbBossHorde` bool@0x0e, 2 `mbCaptainSpawned` bool@0x0f, 3 `mbBossComplete` bool@0x10, 4 `mbHordeSpawned` bool@0x48c, 5 `mBossId` tObjID/u32@0x14, 6 `mActiveHordeWaves` int@0x47c. ⇒ C# writing `mEnabled/mState/…` (fields that don't exist) is wrong-shaped — emit a reflection bitmap over these 7. **Impl:** derive the schema from `AssetData.Parser` (it already maps `cAIDirector`), don't hardcode.
