...
- **`Write<T>` wrapper → little-endian on the wire.** The C++ `Write<T>(BitStream&, T)` wrapper (`RakNet/Types.h:217`) bswaps, then `BitStream::Write<T>` byte-reverses again on an LE host — two swaps cancel, wire is **LE**. Verified 2026-05-28 by raw capture vs the working C++ server, re-confirmed 2026-05-31 (LPU/GameState/deploy all LE on the wire). Darkspore game packets are LE.
