...
Seed rows above are from the 2026-05-31 session; expand as the walk proceeds (steps 1–3 not yet wire-diffed). D-005 verified by C++ source (include-order proof) + matches the existing capture claim; full wire re-confirm pending proper RakNet bit-alignment parse. D-004 client-verify gate pending.
