...
| 07 | ChainVoting | ✅ | [`phases/07-chainvote.md`](phases/07-chainvote.md) | **M4-2 closed 2026-05-24**: `ChainData` 0x151 buffer byte-by-byte audited. `🔒` LE encoding (C# is canonical wire form). One structural divergence in Branch A: C++ tail-6-u32 at `0xED`, C# at `0xEE` (LE rule says C# wins). `mChainSummary[3].playerIndex` clobber bug present both sides. Defaults: C++ seeds 6 enemies + 2 levelNouns; C# seeds 5 enemies + 0 levelNouns (closed by `PopulateFromLevel` only when `--game-path` provided). |
