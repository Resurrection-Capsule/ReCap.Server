...
Provided by `lib/RakNexus`. Triggers on the C# equivalents of `ID_DISCONNECTION_NOTIFICATION` and `ID_CONNECTION_LOST`. Whether RakNexus distinguishes between graceful-FIN and timeout in the `reason` field is unverified — see Milestone 4 audit item in [ROADMAP.md](../ROADMAP.md).
