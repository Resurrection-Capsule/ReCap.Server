...
> **Naming.** The robust C# facade proposed here is named **`AssetDatabase`**, not `NounDatabase`. The C++ `NounDatabase` is a historical misnomer — `Noun` is just **1 of 143** registered AssetData formats; dalkon named the database after the first format he tackled, then bolted on 7 more typed dicts. `AssetData` is the real system. See [`GHIDRA_GROUND_TRUTH.md`](GHIDRA_GROUND_TRUTH.md) and [`FORMAT_COVERAGE.md`](FORMAT_COVERAGE.md). C++ `NounDatabase` references below describe dalkon's actual code; the C# target keeps the generic `AssetDatabase` name (per CLAUDE.md).
...
```csharp
public sealed class AssetDatabase   // singleton or DI scope — supersedes the thin wrapper
{
...
|---|---|---|
| 1 | Add the robust `AssetDatabase` skeleton + warm-up scaffolding alongside the current thin wrapper | new `Services/Assets/` tree, called from `Program.cs:118` |
...
| 8 | Drop the old thin wrapper once everything has migrated; delete generic `_cache` | thin `AssetDatabase` implementation removed |
...
ReCap.Server/
  Services/
    AssetDatabase.cs               — robust facade + warm-up (supersedes thin wrapper)
    Assets/
      Loaders/
        NounLoader.cs
        LevelLoader.cs
...
During migration the thin wrapper is kept under a transitional name (e.g. `LegacyAssetDatabase`) and delegates to the robust `AssetDatabase`; it is deleted at phase 8.

`Domain/Gameplay/ChainData.cs` shrinks dramatically — no more raw `AssetNode` walks; pulls from `LevelDef`.
`Domain/Gameplay/Game.cs` `OnPlayerStart` reads from `LevelDef.MarkerSets.First(...).Loaded` and `AssetDatabase.GetNoun(...)`.
