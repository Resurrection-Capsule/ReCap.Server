...
function setServerStatus(health, maintenance, countdown)
{
    ...
    var innervar;
    if (mServerHealth) {
        innervar = '<div style="color:#FFFFFF;font-size:11px;" align="right">'+localizedkeymap['SERVER_GOOD']+'</div>';
    }
    else if (mServerMaintenance) {
        innervar = '<div style="color:#FFFFFF;font-size:11px;" align="right">'+localizedkeymap['SERVER_UNAVAILABLE']+'</div>';
        mError = true;
    } else {
        innervar = '<div style="color:#FFFFFF;font-size:11px;" align="right">'+localizedkeymap['SERVER_UNAVAILABLE']+'</div>';
        mError = true;
...
function setFrameAsError(key, refreshable) {
    ...
    if (!mError || (mRefresh && !refreshable)) {
        ...
        var supportstring = localizedkeymap['SUPPORT_STRING'];
...
function isGameVersionAtLeast(versionArray) {
  ...
  return Utils.compareVersions(version, versionArray) != 1;
}
...
document.getElementById("Server_condition").innerHTML = 
        '<div style="color:#FFFFFF;font-size:11px;" align="right">CONTACTING</div>';
